...
go_local=$(extract "default_language_version golang" 1 .pre-commit-config.yaml 's/^  golang: "\(.*\)"$/\1/p')
...
same "go.mod toolchain and .pre-commit-config.yaml golang" "$go_toolchain" "$go_local"
